...
class QuickRankingController extends Controller
{
    public $errorFiles = [];
    ...
    {
        // DB::table('cv_files_quick')->truncate();
        // DB::table('cv_rank_quick')->truncate();
        // DB::table('key_weight_quick')->truncate();
        return view('quick_ranking.index')->with('title','CV Ranking');
    }
    ...
    public function rankCV(Request $request)
    {
        ...
        //DB::table('key_weight_quick')->where('process_id', '=', $operationId)->delete();
         ...
         return view('quick_ranking.ranklist')->with(['title' => 'Rank of CVs','cvs'=>$cvs,'operationId'=>$operationId, 'errorFiles'=>$this->errorFiles]);
    }
    ...
    public function processCVFile($file, $destinationPath, $operationId)
    {
      ...
      if($fileExt=='docx' OR $fileExt=='doc' OR $fileExt=='pdf')
      {
        ...
        if($content == "")
        {
            array_push($this->errorFiles, $fileName);
            return;
        }
...
END;
    ...
    public function extractPdf($filePath)
    {
        ...
        $version = $guesser->guess($filePath);
        if($version == "1.7") {
